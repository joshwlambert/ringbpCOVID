...
% Centred, larger, bold header cell for the weighted X columns above. X columns
% cannot be used inside \multicolumn, so the cell is centred by scoping
% \centering to the cell and closing the paragraph with \par before the group
% ends, which avoids \centering redefining the row-terminating \\.
\newcommand{\thcell}[1]{\begingroup\centering\small\bfseries #1\par\endgroup}
...
Since the pandemic the utility of the \texttt{ringbp} model and the possibility of generalising it into a framework for evaluating targeted interventions for outbreak control were recognised. The post-pandemic development that followed affected both assumption and implementation correctness, and may cause substantial differences in model output. Supplementary table \ref{tab:dev-table} outlines each of the important model developments and links to the open-source development record. In this piece we reproduce the analysis of \cite{Hellewell2020} using both the pandemic version of \texttt{ringbp} (v0.1.0) and the current version (v2.0.0) to evaluate whether the results that informed public health decision making would differ today. We quantify differences in outbreak control, measured using the probability of extinction (P(extinct)) and simulated effective reproduction number. Additionally, we compare model output directly before and after each development; this includes changes that cannot influence the results of \citet{Hellewell2020}, because they fall outside the parameter space of that study. We also compare the computational performance of the two versions. See the supplementary material for a full description of the methodology. \\

The original findings of \cite{Hellewell2020} showed that for a SARS-CoV-2 outbreak with a basic reproduction number ($R_0$) of 1.5, isolating cases and a short delay after symptom onset (averaging 3.8 days), and tracing at least 20\% of their contacts would control the majority of outbreaks. To control the majority of outbreaks for $R_0$ of 2.5, more than 60\% of contacts need to be successfully traced, and for $R_0$ of 3.5 more than 90\% of contats need to be traced. The number of initial cases and speed of onset-to-isolation  had effects on outbreak control, with smaller initial clusters much more likely to be contained. Asymptomatic cases and presymptomatic transmission reduced the efficacy of isolation and contact tracing, notably at moderate to high proportions of contacts traced. Reproducing the analysis of \cite{Hellewell2020}, we find the same qualitative patterns of outbreak control (Figure \ref{fig:outbreak-control-r} and \ref{fig:outbreak-control}). \\

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../plots/outbreak_control_r_ci.png}
    \caption{Reproduction of figure 3A in \cite{Hellewell2020} (pandemic version, diamonds) with the results using the current version (circles) of \texttt{ringbp} and confidence intervals overlayed. The percentage of outbreaks that are controlled for scenarios with varying basic reproduction number ($R_0$), at each value of contacts traced. The baseline scenario is 20 initial cases, a short delay to isolation, 15\% of transmission before symptom onset, and 0\% subclinical (asymptomatic) infection. A simulated outbreak is defined as controlled if there are no cases between weeks 12 and 16 after the initial cases.}
    \label{fig:outbreak-control-r}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../plots/outbreak_control.png}
    \caption{Reproduction of figure 4 in \cite{Hellewell2020} (pandemic version, diamonds) with the results using the current version (circles) of \texttt{ringbp} overlayed. The percentage of outbreaks that are controlled for scenarios at each value of contacts traced, with varying outbreak parameters in each subplot. (A) The initial number of cases, (B) delay from symptom onset to isolation, (C) percentage of transmission before symptom onset, (D) the percentage of subclinical (asymptomatic) cases. The baseline scenario is R0 of 2.5, 20 initial cases, a short delay to isolation, 15\% of transmission before symptom onset, and 0\% subclinical infections. A simulated outbreak is defined as controlled if there are no cases between weeks 12 and 16 after the initial cases.}
    \label{fig:outbreak-control}
\end{figure}

Quantitative differences between the pandemic and current \texttt{ringbp} version were found ... \\
...
\footnotesize
\begin{xltabular}{\textwidth}{|L{0.4}|L{1.65}|L{1.65}|L{0.8}|L{0.75}|L{0.75}|}
...
\label{tab:dev-table} \\
\hline
\thcell{Label} & \thcell{Problem description} & \thcell{Change description} & \thcell{Change type} & \thcell{GitHub issue(s)} & \thcell{GitHub Pull request(s)} \\
\hline\hline
\endfirsthead
\hline
\thcell{Label} & \thcell{Problem description} & \thcell{Change description} & \thcell{Change type} & \thcell{GitHub issue(s)} & \thcell{GitHub Pull request(s)} \\
...
\hline
\endlastfoot
\multicolumn{6}{|c|}{\normalsize\bfseries1 Intervention} \\
\hline
...
1b & If quarantine was active, traced infectees were isolated at their infectors isolation time, even if this event time was after the time an individual became symptomatic and went into isolation after their onset-to-isolation delay. & When qurantine is active, infectees are isolated at the earliest of their infector's isolation time or the onset-to-isolation delay after becoming symptomatic. In theory, the latter could be earlier due to variability in sampling isolation delays. (N.B. quarantine was not used in \cite{Hellewell2020}). & Bug\slash Model choice & \ghissue{102} & \ghpr{107} \\
 ...
 \hline
 \multicolumn{6}{|c|}{\normalsize\bfseries2 Epidemic model} \\
 \hline
 ...
 \hline
 \multicolumn{6}{|c|}{\normalsize\bfseries(3) User interface and flexibility} \\
 \hline
 ...
 \hline
 \multicolumn{6}{|c|}{\normalsize\bfseries(4) Performance} \\
 \hline
 ...
 \hline
 \multicolumn{6}{|c|}{\normalsize\bfseries(5) Testing and correctness} \\
 \hline
 ...
 \hline
 \multicolumn{6}{|c|}{\normalsize\bfseries(6) Documentation} \\
 \hline
...
\subsubsection*{Hellewell et al. (2020) parameters}

Throughout the supplementary methods we refer to the parameters used in \cite{Hellewell2020}. Some of these parameters were extracted from the literature for COVID-19, or from related pathogens (e.g. SARS-CoV-1). Please see the table in \cite{Hellewell2020} for those sources. Here we provide the parameter space for that study for reference:

\begin{table}[h!]
\centering
  \caption{Parameters in \cite{Hellewell2020}. Weibull parameters are shape ($k$) and scale ($\lambda$). The basic reproduction numbers ($R_0$ and $R_0^{iso}$) and the dispersion parameter ($k$) parameterise the negative binomial offspring distributions. Quarantine is not active in \cite{Hellewell2020}, so traced contacts are only isolated once they become symptomatic.}
  \label{tab:hellewell-params}
 \begin{tabularx}{\textwidth}{|L{0.85}|L{1.15}|}
 \hline
 \thcell{Parameter} & \thcell{Value} \\
 \hline\hline
 Initial cases & 5, 20, 40 \\
 \hline
 Percentage of contacts traced & 0\%, 20\%, 40\%, 60\%, 80\%, 100\% \\
 \hline
 Basic reproduction number in community ($R_0$) & 1.5, 2.5, 3.5 \\
 \hline
 Basic reproduction number in isolation ($R_0^{iso}$) & 0 \\
 \hline
 Dispersion ($k$) & 0.16 \\
 \hline
 Asymptomatic (subclinical) case percentage & 0\%, 10\% \\
 \hline
 Presymptomatic transmission percentage & 1\%, 15\%, 30\% \\
 \hline
 Onset-to-isolation delay & Short (SARS-like): Weibull, $k$ = 1.651524, $\lambda$ = 4.287786 (mean = 3.83 days). Long (Wuhan-like): Weibull, $k$ = 2.305172, $\lambda$ = 9.483875 (mean = 8.40 days). \\
 \hline
 Incubation period & Weibull, $k$ = 2.322737, $\lambda$ = 6.492272 (mean = 5.75 days) \\
 \hline
 Quarantine & inactive \\
 \hline
 Maximum number of days to simulate & 365 \\
 \hline
 Maximum number of cumulative cases to simulate & 5,000 \\
 \hline
 \end{tabularx}
\end{table}
...
To examine the impact of the change in implementation of quarantine between pandemic (v0.1.0, commit \ghcommit{238b420}) and current (v2.0.0) versions of \texttt{ringbp} we run a comparison of the \cite{Hellewell2020} analysis, but with quarantine active (N.B. quarantine is inactive in all \cite{Hellewell2020} simulations, Table \ref{tab:hellewell-params}). We compare the difference in the probability of outbreak extinction and the effective reproduction number from each simulation version. \\
